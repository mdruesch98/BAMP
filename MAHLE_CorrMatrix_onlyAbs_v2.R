...
cor_heat_MTD <- ggplot(data = melted_cormat_MTD, aes(Var2, Var1, fill = value))+
  ...
  ggtitle("MTD with NA Absolutes only")

cor_heat_MTD

# Save to working directory
ggsave("cormat_MTD_onlyAbs.png", plot = cor_heat_MTD, width = 10, height = 8, units = "in", dpi = 600)
ggsave("cormat_MTD_onlyAbs.jpg", plot = cor_heat_MTD, width = 10, height = 8, units = "in", dpi = 600)
...
cor_heat_MTD_noNA <- ggplot(data = melted_cormat_MTD_noNA, aes(Var2, Var1, fill = value))+
  ...
  coord_fixed()+
  theme(plot.title = element_text(size = 20, face = "bold", margin = margin(b = 20, unit = "pt")))+
  ggtitle("MTD without NA Absolutes only")

cor_heat_MTD_noNA

# Save to working directory
ggsave("cormat_MTD_noNA_onlyAbs.png", plot = cor_heat_MTD_noNA, width = 10, height = 8, units = "in", dpi = 600)
ggsave("cormat_MTD_noNA_onlyAbs.jpg", plot = cor_heat_MTD_noNA, width = 10, height = 8, units = "in", dpi = 600)
...
cor_heat_YTD <- ggplot(data = melted_cormat_YTD, aes(Var2, Var1, fill = value))+
  ...
  coord_fixed()+
  theme(plot.title = element_text(size = 20, face = "bold", margin = margin(b = 20, unit = "pt")))+
  ggtitle("YTD with NA Absolutes only")

cor_heat_YTD

# Save to working directory
ggsave("cormat_YTD_onlyAbs.png", plot = cor_heat_YTD, width = 10, height = 8, units = "in", dpi = 600)
ggsave("cormat_YTD_onlyAbs.jpg", plot = cor_heat_YTD, width = 10, height = 8, units = "in", dpi = 600)
...
cor_heat_YTD_noNA <- ggplot(data = melted_cormat_YTD_noNA, aes(Var2, Var1, fill = value))+
  ...
  coord_fixed()+
  theme(plot.title = element_text(size = 20, face = "bold", margin = margin(b = 20, unit = "pt")))+
  ggtitle("YTD without NA Absolutes only")

cor_heat_YTD_noNA

# Save to working directory
ggsave("cormat_YTD_noNA_onlyAbs.png", plot = cor_heat_YTD_noNA, width = 10, height = 8, units = "in", dpi = 600)
ggsave("cormat_YTD_noNA_onlyAbs.jpg", plot = cor_heat_YTD_noNA, width = 10, height = 8, units = "in", dpi = 600)
